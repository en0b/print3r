...
class PrinterHelper:
        ...
        with self._open() as printer:
            for ln in lines:
                if cancel_flag_getter and cancel_flag_getter():
                    break
                self._with_retry(printer.out, sanitize_for_printer(ln))
    ...
    def print_image(self, img: Image.Image, feed_lines: int = 10):
        img = ensure_filename(img, "buffer.png")
        with self._open() as printer:
    ...
    def print_image_bandwise(
        ...
        """
        # Ensure filename for libs that log filenames
        img = ensure_filename(img, "bandwise.png")

        # Make sure mode/width match what the printer expects
        if img.mode != "1":
            img = img.convert("1", dither=Image.FLOYDSTEINBERG)
        if img.width != WIDTH_PIXELS:
            factor = WIDTH_PIXELS / img.width
            new_size = (WIDTH_PIXELS, max(1, int(round(img.height * factor))))
            img = img.resize(new_size, RESAMPLE_FILTER).convert("1")
        ...
        with self._open() as printer:
            ...
            while y < H:
                ...
                # Compute darkness ratio to adapt sleep: 0 (white) .. 1 (fully black)
                # Use histogram on grayscale to avoid numpy dependency.
                g = band.convert("L")
                ...
                # Sleep a bit longer for dark bands so the head can keep up
                time.sleep(base_sleep + coverage * dark_bonus)

                y += band_height

            # Final feed to clear the cutter area
            self.safe_feed(printer, feed_lines)
...
class ThermalPrintTool:
        ...
        while True:
            if event.wait(1):
                if self.printer_port is None:
                    ...
                    continue
                try:
                    with ThermalPrinter(port=self.printer_port, heat_time=HEAT_TIME) as printer:
                        printer.image(ensure_filename(self.display_image, "processed.bmp"))
                        for _ in range(10):
                            printer.out("")
    ...
    def cancel_print(self):
        """Cancels ongoing text prints (checked between line blocks)."""
    ...
    def print_meme(self):
        ...
        try:
            ...
            self.printer_helper.print_image_bandwise(
                img_1bit,
                band_height=64,         # try 32 on very sensitive printers
                base_sleep=0.025,       # base pause per band
                dark_bonus=0.20,        # extra pause * darkness ratio
                feed_lines=8,
